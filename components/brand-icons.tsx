 ...
 *
 * lucide-react dropped its brand icons, and the neutral substitutes were worse
 * than useless — a briefcase does not read as LinkedIn. These are the official
 * marks, inlined rather than pulled from a package: six paths is far less
 * weight than a dependency, and it keeps the bundle free of an icon set we
 * would use six glyphs from.
 *
 * Paths from Simple Icons (https://simpleicons.org), which releases its icon
 * data under CC0. The marks themselves remain trademarks of their owners and
 * are used here only to link to the corresponding profile, which is the
 * nominative use those brand guidelines allow.
 *
 * `currentColor` throughout, so they inherit hover and focus states like any
 * other icon, and `aria-hidden` because the surrounding link carries the label.
 */
type Props = { className?: string };

const svg = (path: string) =>
  function BrandIcon({ className }: Props) {
    return (
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
...
export const InstagramIcon = svg(
  ...
  + '1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 '
  ...
  + '6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 '
  + '4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 '
...
export const YouTubeIcon = svg(
  ...
  + '3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 '
...
export const TelegramIcon = svg(
  'M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 '
  + '12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 '
  + '1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 '
